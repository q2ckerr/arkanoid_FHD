...
from arkanoid.sound import play_powerup
...
class CatchPowerUp(PowerUp):
    ...
    A ball is released by pressing the spacebar (keyboard) or the
    A / Cross button (gamepad). If the ball is not released within
    a timeout period, it is released automatically.
    """

    # Maximum number of frames the ball stays anchored to the paddle
    # before being released automatically.  At 60 fps this equals 5 s.
    CATCH_TIMEOUT = 300

    def __init__(self, game, brick):
        super().__init__(game, brick, 'powerup_catch')

        # The per-frame polling sprite that watches the gamepad and
        # releases a caught ball on the A / Cross button. Created
        # lazily in _activate() so the sprite is only in the game's
        # sprite list for as long as the powerup is active.
    ...
    def _activate(self):
        ...
        # Add a per-frame polling sprite so the A / Cross gamepad
        # button can also release a caught ball. KEYUP events are
        # only generated by the keyboard, so we need to poll the
        # gamepad directly for this action.
        self._gamepad_listener = _GamepadCatchReleaseListener(self.game, self)
    ...
    def deactivate(self):
        ...
        # Remove the per-frame gamepad polling sprite from the game.
        if self._gamepad_listener is not None:
            if self._gamepad_listener in self.game.sprites:
    ...
    def tick(self):
        """Called every frame by the gamepad listener sprite.

        Increments the catch timer and auto-releases the ball when
        the timeout is reached.
        """
        if self._ball_caught:
            self._catch_frames += 1
            if self._catch_frames >= self.CATCH_TIMEOUT:
                for ball in self.game.balls:
                    ball.release()
                self._ball_caught = False
                self._catch_frames = 0
...
class _GamepadCatchReleaseListener(pygame.sprite.Sprite):
    """Per-frame polling sprite that releases a caught ball on the
    A / Cross gamepad button.

    The CatchPowerUp uses a KEYUP event handler to release the ball
    when the user presses Space on the keyboard. Gamepad button
    presses do NOT generate KEYUP events, so the powerup adds one
    of these listeners to ``self.game.sprites`` on activation and
    removes it on deactivation. The listener's ``update()`` is
    invoked by ``Game._update_sprites()`` every frame, giving us a
    reliable per-frame poll.

    The sprite is never visible (and is drawn at a degenerate 1x1
    rect) so the render loop blits a single pixel of background
    over an empty area - this has no visual side effects.
    """

    def __init__(self, game, catch_powerup=None):
        super().__init__()
        self.game = game
        self._catch_powerup = catch_powerup
        # A 1x1 placeholder surface for the render loop. The sprite
        # is never blitted (visible = False) so this is purely
        # cosmetic.
    ...
    def update(self):
        ...
        if get_gamepad().fire_pressed:
            for ball in self.game.balls:
                ball.release()
            if self._catch_powerup is not None:
                self._catch_powerup._ball_caught = False
                self._catch_powerup._catch_frames = 0

        # Tick the catch timeout timer.
        if self._catch_powerup is not None:
            self._catch_powerup.tick()
